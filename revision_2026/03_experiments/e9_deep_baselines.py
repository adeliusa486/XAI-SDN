"""E9 - Deep-learning baselines under the same CPU protocol (R4.2, R1.5, R6.5, R4.3).

Reviewer 4 asks for a comparison against "modern deep learning detectors (CNN,
LSTM, autoencoder, GNN, transformer) under the same protocol". Reviewer 6 insists
the protocol be identical for every model. Reviewer 1 insists everything be timed
...
def main() -> int:
    ...
    # alternative would be to omit the baseline the reviewer asked for.
    ben = Xtr[ytr == 0]
    ae_note_extra = ""
    if len(ben) < 2000:
