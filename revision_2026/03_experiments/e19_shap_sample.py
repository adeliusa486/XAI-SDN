...
The submitted attribution figure was drawn from the model of the submitted
pipeline and ranks FIN_Flag_Count third, which cannot be right for a column that
is constant across the whole partition (Sec. VII-C). The figure has to be
redrawn from the corrected model, and a beeswarm needs one attribution vector
per flow, which E6 does not persist because it only needs the mean.
