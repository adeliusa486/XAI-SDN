...
reviewer concern:
  ...
  * the entropy engine is instrumented so that the H_ttl defect (R7.2) can be
  ...
  * both the temporal split described in the manuscript and the stratified split
    ...
    one actually produced the submitted numbers (R7.8);
  * the running-sum entropy update is exposed in three numerical variants so the
    drift study (R6.4) can target the form that actually executes.
...
# Submitted paper's entropy set. H_ttl is the defective one (R7.2).
...
# --------------------------------------------------------------------------- #
# Rolling entropy: three numerical variants for the drift study (R6.4)
# --------------------------------------------------------------------------- #
...
def entropy_streams(df: pd.DataFrame, repaired: bool) -> tuple[list[list[Any]], list[str]]:
    ...
    manuscript can publish a complete feature specification (R7.2, R7.3).
    """
    n = len(df)
