"""E12 - Precision-recall based threshold selection (R6.7, R1.3).

Reviewer 6: "The detection threshold tau=0.7 is selected based on a sweep that
claims 'zero FNR below tau=0.5', but in highly imbalanced datasets, FNR is
misleading; the focus must be on the Precision-Recall curve."

The objection is correct. Under 99:1 prevalence, FNR is computed over the
...
This experiment:
  * builds a chronological train / validation / test partition so that the
    threshold is never selected on test data (this also answers R7.8);
...
def main() -> int:
    ...
    # ---- threshold sweep, expressed the way the reviewer asked -------------
