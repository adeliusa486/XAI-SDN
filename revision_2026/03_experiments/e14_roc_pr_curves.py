"""E14 - ROC and precision-recall curves for every method (R7.6, R6.7).

Reviewer 7 noted that the submitted paper's Section VI-E discusses ROC curves
for the baselines but contains no figure at all. This experiment produces the
curve data for every model evaluated in E1 and E9, and pairs each ROC curve with
a precision-recall curve, because under 99:1 prevalence the ROC curve flatters
every classifier and the PR curve does not.
...
def main() -> int:
    ...
    out["interpretation_note"] = (
        ...
        "curve, which is the point Reviewer 6 raised about threshold selection.")
