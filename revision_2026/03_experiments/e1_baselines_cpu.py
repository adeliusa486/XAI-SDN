"""E1 - Unified CPU-only classical baseline comparison (R1.4, R1.5, R1.6, R6.5).

Two reviewer complaints are answered together.

Reviewer 1 observed that Table 8 timed the DNN on a GPU and everything else on a
CPU, making the latency and throughput columns incomparable. Every model here
runs on the same CPU, in the same harness, on the same machine, with the same
thread count, and the torch build in this environment has no CUDA at all, so a
GPU measurement is not merely avoided but impossible.

Reviewer 6 observed that comparing baselines trained on 10,000 class-balanced
samples against a Random Forest trained on 2.5M imbalanced flows is meaningless.
So every model is run under BOTH protocols, and the two are never mixed:
