"""E10 - Explanation trigger policy and budgeted SHAP throughput (R6.3, R2.4, R7.1).

Reviewer 6: "since 99.14% of the test traffic is DDoS, Algorithm 1 will trigger
SHAP for almost every flow, creating a massive bottleneck at the SDN controller."

That is correct, and the submitted paper's 1,953 flows/s figure concedes it
without addressing it. This experiment turns the objection into a contribution
by defining the trigger formally, measuring the bottleneck, and then measuring
three mitigations that recover throughput while keeping the audit trail.

Explanation trigger, defined once and used everywhere afterwards (R2.4):

    explain(f) = 1  iff  y_hat(f) = 1  AND  s(f) >= tau  AND  B(f) = 1

where y_hat is the predicted label, s the DDoS score, tau the detection
threshold, and B the explanation-budget admission decision. The submitted paper
implicitly set B = 1 always, which is the configuration the reviewer criticises.

Policies measured:
  P0  always            - B = 1 for every alert (the submitted configuration)
