"""E0 - Reproduce the submitted paper's headline numbers on the real dataset.

Gate for every other experiment. Runs BOTH candidate split protocols:

  * 'stratified' - random stratified 70/30 (what the repository code implements)
  * 'temporal'   - all training flows precede all test flows (what the manuscript claims)

and reports which one reproduces the submitted numbers. Also records the
H_ttl variance needed by E16 (R7.2) and caches the cleaned matrices so the
1.87 GB CSV is parsed exactly once.
"""
...
def main() -> int:
    ...
    # --- H_ttl defect evidence for E16 / R7.2 -------------------------------
