\begin{tikzpicture}[
    ...
    module/.style={draw=xsdnteal, thick, fill=white, align=center, inner sep=2mm, minimum height=1.2cm, font=\sffamily},
    ...
    host/.style={draw=xsdngray, thick, fill=white, rectangle, minimum width=0.8cm, minimum height=0.6cm, align=center, font=\sffamily\scriptsize},
...
\node[dashboard] (dash) at (3, 4.8) {
    ...
    Threat Alerts \& SHAP Explanations
};

% ----------------------------------------------------
% CONTROL PLANE
% ----------------------------------------------------
% Main SDN Controller Box
\node[controllerbox, minimum width=10.5cm, minimum height=5cm] (controller) at (3, 0) {};
...
\node[module, text width=3.0cm] (entropy) at (-0.8, 0.8) {\textbf{Rolling Entropy}\\{\scriptsize expected amortized $\mathcal{O}(1)$}};
\node[module, text width=2.4cm] (rf) at (3, 0.8) {\textbf{Random Forest}\\Classifier};
\node[module, text width=2.2cm] (shap) at (6.8, 0.8) {\textbf{SHAP}\\TreeExplainer};

\draw[arrow] (entropy) -- (rf);
\draw[arrow] (rf) -- (shap);

% Core Controller Functions
\node[module, text width=2.2cm] (topo) at (-0.8, -1.2) {Topology\\Management};
\node[module, text width=2.2cm] (flow) at (3, -1.2) {Flow\\Management};
\node[module, text width=2.2cm] (stat) at (6.8, -1.2) {Statistics\\Collection};
