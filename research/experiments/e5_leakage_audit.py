"""E5 - Leakage and dataset-artifact audit.

Near-perfect accuracy on this benchmark may reflect dataset artifacts rather
than attack signal, and removing the top three attributed features does not
test that. This experiment settles the question with evidence rather than
assertion.
...
Seven parts:
  ...
  5. progressive removal - top 1/3/5/10/20 suspects and the ENTIRE flagged set,
     which is the strongest form of the test;
...
def bootstrap_ci(y_true, y_pred, y_score, n_boot=N_BOOT, seed=42) -> dict:
    """Percentile bootstrap CIs. This experiment provides FPR/FNR confidence intervals."""
...
def distribution_checks(X, y, names, rng) -> pd.DataFrame:
    """Per-feature class-conditional divergence (feature-distribution checks)."""
...
def main() -> int:
    ...
    # the strongest form of the test: remove ALL objectively-flagged suspects
    if suspects:
